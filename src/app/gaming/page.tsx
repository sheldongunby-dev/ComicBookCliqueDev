...
export const metadata: Metadata = buildMetadata({
    ...
    description: "The latest video game news, reviews, and coverage from Comic Book Clique.",
    path: "/gaming",
});

export default async function GamingPage() {
    // Fetch all news, filter by category
    const allNews = await getNews();
    const gamingNews = allNews
        .filter(n => n.category === 'gaming')
            ...
            <div className="relative bg-cbc-darker border-b border-cbc-border pt-32 pb-12">
                ...
                <div className="relative max-w-screen-2xl mx-auto px-4 sm:px-6">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="font-label text-xs font-semibold tracking-[0.25em] uppercase text-cbc-cyan">Gaming</span>
                    </div>
                    <h1 className="font-display text-display-xl text-cbc-white leading-none uppercase">Player One</h1>
                    <p className="text-cbc-muted mt-3 max-w-lg leading-relaxed">
                        Industry news, reviews, and the games we can't stop playing right now.
                    </p>
                </div>
            </div>
                ...
                <FadeIn>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {gamingNews.map((item) => {
                            const i = item as any;
                            return (
                                <Link key={i.id} href={`/news/${i.slug}`} className="group card-cinematic hover:bg-cbc-cyan/5 overflow-hidden block border border-cbc-border/50">
                                    {i.heroImage && (
                                    ...
                                    <div className="p-4">
                                        <h3 className="font-heading font-semibold text-cbc-white line-clamp-2 leading-snug group-hover:text-cbc-cyan transition-colors duration-300 mb-2">
                                            {i.title}
                                        </h3>
                    ...
                    {gamingNews.length === 0 && <p className="text-cbc-muted mt-8 text-center">No posts yet.</p>}
                </FadeIn>
            </SectionShell>
        </>
    );
